...
- [x] **FÁZE 7** – Nasazení _(KOMPLETNÍ: 7.2 produkční Dockerfiles api+web(nginx)+docker-compose.prod.yml (Traefik labels, hardening), 7.3 deploy.yml na prod compose, 7.4 monitoring + 7.5 backup skript, docs/deployment.md. Opraveny 3 prod chyby (shared bundling, migrate CLI, nesting migrací). Server připraven uživatelem, **první úspěšný deploy 2026-06-11 (run #3, commit d7b3103)**, push trigger zapnut – další merge do main se nasazují automaticky)_
